...
const StationaryHistory: FC<IProps> = ({ calculation }) => {
  const [active, setActive] = useState<string>(EStationaryTypes.Book);
          ...
          <Wrapper>
            <Segmented
              ...
              options={[
                {
                  label: (
                    <Title isActive={active == EStationaryTypes.Book}>
                      Student’s book
                      <AntdBadge
                        content={
                          calculation?.stationaryHistory?.[
                            EStationaryTypes.Book
                          ]?.length || 0
                        }
                        showZero
                      />
                  ...
                  value: EStationaryTypes.Book,
                  children: children(EStationaryTypes.Book),
                },
                {
                  label: (
                    <Title isActive={active == EStationaryTypes.CopyBook}>
                      Copybook
                      <AntdBadge
                        content={
                          calculation?.stationaryHistory?.[
                            EStationaryTypes.CopyBook
                          ]?.length || 0
                        }
                        showZero
                      />
                  ...
                  value: EStationaryTypes.CopyBook,
                  children: children(EStationaryTypes.CopyBook),
                },
              ]}
              initValue={active}
              block
