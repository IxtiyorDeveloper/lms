...
const TaxChart = ({ total }: { total: ISalaryTotal }) => {
  const data = useMemo(() => {
    return [
      {
        name: "Jan",
        total: total?.card_tax?.tax?.[0]?.total,
        department: total?.card_tax?.tax?.[0]?.department?.name,
        color: "blue",
      },
      {
        name: "Feb",
        total: total?.card_tax?.tax?.[1]?.total,
        department: total?.card_tax?.tax?.[1]?.department?.name,
        color: "midori",
      },
      {
        name: "Mar",
        total: total?.card_tax?.tax?.[2]?.total,
        department: total?.card_tax?.tax?.[2]?.department?.name,
        color: "primary",
      },
    ];
  }, [total]);
