...
const ByCategories = () => {
  ...
  const getBySubLevel = (level: string) => {
    if (data?.lostCategory) {
      const lostCategory = data?.lostCategory?.filter((fresh) =>
        fresh.type.includes(level),
      );
      const total = lostCategory?.reduce((acc, cer) => {
        return acc + +cer.count;
      }, 0);

      const grouped = _.groupBy(lostCategory, "label");
