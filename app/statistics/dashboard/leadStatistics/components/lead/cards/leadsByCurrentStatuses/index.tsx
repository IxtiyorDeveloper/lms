...
const LeadsByCurrentStatuses = () => {
  ...
  const getAllData = () => {
    if (!!data?.leadByStatus) {
      const leadByStatus = data?.leadByStatus;
      const total = leadByStatus?.reduce((acc, cer) => {
      ...
      leadByStatus?.map((leadStatus, index) => {
        dataList.push({
  ...
  return (
    <Wrapper>
      <StatisticsCard title="Leads by current statuses" isLoading={isLoading}>
...
export default LeadsByCurrentStatuses;
