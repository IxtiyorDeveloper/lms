...
const LeadsRegisteredByAdmins = () => {
  const router = useRouter();

  const { isLoading, data } = useLeadStatistics({
    query_params: {
      type: "registeredByAdmin",
  ...
  const getAllData = () => {
    if (!!data?.registeredByAdmin) {
      const registeredByAdmin = data?.registeredByAdmin;
      const total = registeredByAdmin?.reduce((acc, cer) => {
        return acc + +cer.count;
      }, 0);

      const grouped = _.groupBy(registeredByAdmin, "name");
  ...
  return (
    <Wrapper>
      <StatisticsCard title="Registered leads by admins" isLoading={isLoading}>
...
export default LeadsRegisteredByAdmins;
