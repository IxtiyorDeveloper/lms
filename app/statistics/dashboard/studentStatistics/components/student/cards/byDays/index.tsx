...
const ByTime = () => {
  ...
  const example = _.uniqBy(data?.dayShare, "status_label");
  ...
  const getAllData = () => {
    if (data?.dayShare) {
      const dayShare = data?.dayShare.filter((v) =>
        filter.includes(v.status.toLowerCase()),
      );
      const total = dayShare?.reduce((acc, cer) => {
        return acc + +cer.count;
      }, 0);

      const grouped = _.groupBy(dayShare, "label");
