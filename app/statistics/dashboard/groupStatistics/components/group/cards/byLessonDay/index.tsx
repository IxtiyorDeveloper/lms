...
const ByLessonDay = () => {
  ...
  const { isLoading, data } = useGroupStatistics({
    query_params: {
      ...
      branches: branches,
      group_form:
        router.query?.groupsByDays !== "0"
          ? router.query?.groupsByDays
          : groupForm === "0"
            ? null
            : groupForm,
    },
  ...
  const getAllData = () => {
    if (data?.byLessonDay) {
      const groupStatusShare = data?.byLessonDay;
  ...
  return (
    <Wrapper>
      <StatisticsCard
        ...
        initialTabValue="0"
        menu={menuTabs()}
        routerKey="groupsByDays"
        title="Groups by days"
        isLoading={isLoading}
      />
    </Wrapper>
  );
