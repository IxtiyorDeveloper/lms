...
const ByLevel = () => {
  ...
  const { isLoading, data } = useGroupStatistics({
    query_params: {
      ...
      branches: branches,
      group_form:
        router.query?.groupsByLevel !== "0"
          ? router.query?.groupsByLevel
          : groupForm === "0"
            ? null
            : groupForm,
    },
  ...
  const getAllData = () => {
    if (data?.byLevel) {
      const groupStatusShare = data?.byLevel;
      const grouped = _.groupBy(groupStatusShare, "name");
