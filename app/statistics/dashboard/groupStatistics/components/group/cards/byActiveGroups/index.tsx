...
const ByActiveGroupsStatuses = () => {
  ...
  const { isLoading, data } = useGroupStatistics({
    query_params: {
      ...
      branches: branches,
      group_form:
        router.query?.activeGroupStatuses !== "0"
          ? router.query?.activeGroupStatuses
          : groupForm === "0"
            ? null
            : groupForm,
    },
  ...
  const getAllData = () => {
    if (data?.byActiveStatus) {
      const groupStatusShare = data?.byActiveStatus.filter(
        (e) => e.state !== "100",
      );
  ...
  return (
    <Wrapper>
      <StatisticsCard
        ...
        initialTabValue={0}
        menu={menuTabs()}
        routerKey="activeGroupStatuses"
        title="Active groups by statuses"
        isLoading={isLoading}
      />
    </Wrapper>
  );
...
export default ByActiveGroupsStatuses;
