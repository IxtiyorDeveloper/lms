...
const ByTime = () => {
  ...
  const { isLoading, data } = useGroupStatistics({
    query_params: {
      ...
      branches: branches,
      group_form:
        router.query?.groupsByTime !== "0"
          ? router.query?.groupsByTime
          : groupForm === "0"
            ? null
            : groupForm,
    },
  ...
  const getAllData = () => {
    if (data?.byLessonTime) {
      const groupStatusShare = data?.byLessonTime;
      ...
      Object.keys(grouped).map((key, index) => {
        ...
        dataList.push({
          name: !Array.isArray(key) ? key.slice(0, 5) : key,
          value: count,
          total,
  ...
  return (
    <Wrapper>
      <StatisticsCard
        ...
        initialTabValue={0}
        menu={menuTabs()}
        routerKey="groupsByTime"
        title="Groups by time"
        isLoading={isLoading}
      />
    </Wrapper>
  );
